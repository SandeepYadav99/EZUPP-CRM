...
const Cascader = ({
  ...
  const getSelectedValues = useCallback((arr, options, level = 0, res = []) => {
    ...
    const tempIndex = options.findIndex(
      (val) => val.value === arr[level].value
    );
    if (tempIndex < 0) {
      return [];
    }
  ...
  const makeStringValue = useCallback(
    (arr) => {
      let str = "";
  ...
  const getSelectedArr = useCallback(
    (selected) => {
      const arr = [];
  ...
  const renderItems = useMemo(() => {
    const arr = [];
    arr.push(
      <CascaderItem handleClick={handleItemClick} level={0} options={options} />
    );
    ...
    return arr;
  }, [selected, handleItemClick, options]);

  const renderMenu = useMemo(() => {
    if (isOpen) {
      return (
        <div
          className={styles.cascaderCont}
          ref={(ref) => {
            divRef.current = ref;
          }}
        >
          <div>
            <div
              className={csx(
                styles.antCascaderMenus,
                styles.cascaderMenusPlacementBottomLeft
              )}
              style={isOpen ? { left: `${pos[0]}px`, top: `${pos[1]}px` } : {}}
            >
              <div>{renderItems}</div>
            </div>
          </div>
