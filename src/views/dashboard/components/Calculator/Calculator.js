...
const Calculator = () => {
  ...
  const handleButtonClick = (value) => {
    const operators = {
      ...
      const displayValue = operator === '*' ? '×' : operator === '/' ? '÷' : operator;
  ...
  const handleCalculate = () => {
    try {
    const evaluatedExpression = inputValue.replace(/÷/g, '/').replace(/×/g, '*');
...
<div >
  ...
  <div className={styles.value} style={{ fontSize: '38px', color: 'white', textAlign: 'right', fontFamily: 'Arial', fontWeight: 'bold', marginRight: '10px',  wordBreak: 'break-all' }}>{result !== '' ? result : inputValue || '0'}</div>
  </div>
  <div >
  ...
  <div style={{ display: 'flex', alignItems: 'left' }}>
    ...
    <Button className={`${styles.buttonH}`} onClick={handleCalculate} style={{ height: '188%', alignSelf: 'stretch' }}>=</Button> </div>
  </div>
