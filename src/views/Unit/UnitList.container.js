...
import {Add, InputRounded as EditIcon} from '@mui/icons-material';
...
import {
    actionFetchUnit,
    actionChangePageUnit,
    actionChangeStatusUnit,
    actionFilterUnit,
    actionResetFilterUnit,
    actionSetPageUnit,
    actionCreateUnit,
    actionUpdateUnit, actionDeleteUnit
...
class UnitList extends Component {
    ...
    componentDidMount() {
        ...
        // const request = serviceFetchTourUnits();
        // request.then((data)=> {
        //     if(!data.error){
        //         this.setState({
    ...
    _renderCreateForm () {
        if (CreateProvider == null) {
            // import CreateProvider from './Create.container';
            CreateProvider = require('./Unit.view').default;
        }
    ...
    _handleDataSave(data, type) {
        // this.props.actionChangeStatus({...data, type: type});
        if (type == 'CREATE') {
            this.props.actionCreateUnit(data)
        } else {
            this.props.actionUpdateUnit(data)
        }
        this.setState({
            side_panel: !this.state.side_panel,
            edit_data: null,
        ...
        return (
            <div>
                ...
                <SidePanelComponent
                    ...
                    title={'New Unit'} open={this.state.side_panel} side={'right'}>
...
function mapStateToProps(state) {
    return {
        data: state.unit.present,
        total_count: state.unit.all.length,
        currentPage: state.unit.currentPage,
        serverPage: state.unit.serverPage,
        sorting_data: state.unit.sorting_data,
        is_fetching: state.unit.is_fetching,
        query: state.unit.query,
        query_data: state.unit.query_data,
    };
}

function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        actionFetchData: actionFetchUnit,
        actionSetPage: actionSetPageUnit,
        actionResetFilter: actionResetFilterUnit,
        actionSetFilter: actionFilterUnit,
        actionChangeStatus: actionChangeStatusUnit,
        actionCreateUnit: actionCreateUnit,
        actionUpdateUnit: actionUpdateUnit,
        actionDelete: actionDeleteUnit
    }, dispatch);
}
