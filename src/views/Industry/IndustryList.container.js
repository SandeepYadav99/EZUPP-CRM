...
class IndustryList extends Component {
  ...
  renderStatus(status) {
    ...
    if (status === "PENDING") {
      return (
        <span className={classNames("status", className)}>Coming Soon</span>
      );
    }
    return (
  ...
  _handlePreviewToggle(data) {
    window.open(
      `http://91.205.173.97:8093/industry/${data.slug}-${data.id}`, // + data.id
      "_blank"
    );
  }
        ...
        render: (temp, all) => (
          <div>
            ...
            <IconButton
              ...
              color="secondary"
              onClick={this._handlePreviewToggle.bind(this, all)}
            >
              <RemoveRedEyeOutlined fontSize={"small"} />
            </IconButton>
            <Button
              onClick={this._handleCategories.bind(this, all)}
              color={"primary"}
            >
              Categories
            </Button>
